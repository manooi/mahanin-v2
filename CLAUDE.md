...
- `src/components/os94/Window.astro` — the window chrome (title bar, min/close buttons, body variants) used 9× by the hub; geometry arrives as CSS custom properties (`--x/--y/--w/--h/--z`)
...
5. Keep multi-line inline elements (`<a>`, `<strong>`, `<span>`) on one line in `.astro` markup — Astro collapses surrounding whitespace (known repo gotcha).
